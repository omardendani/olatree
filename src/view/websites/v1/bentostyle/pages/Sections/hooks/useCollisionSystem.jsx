...
export const useCollisionSystem = (gridConfig) => {
    return useMemo(() => {
        ...
        const searchInDirection = (element, direction, occupiedSpaces) => {
            ...
            for (let step = 1; step <= GRID_CONFIG.MAX_SEARCH_STEPS; step++) {
                switch (direction) {
                ...
                if (newX < 0 || newX + element.width > GRID_CONFIG.MAX_COLUMNS || newY < 0) {
                    break;
                }
        ...
        const findFirstFreePosition = (element, occupiedSpaces) => {
            for (let y = 0; y < 20; y++) {
                for (let x = 0; x <= GRID_CONFIG.MAX_COLUMNS - element.width; x++) {
